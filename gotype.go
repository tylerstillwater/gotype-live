...
func parseStdin(filename string) (*ast.File, error) {
...
func parseFiles(filenames []string) ([]*ast.File, error) {
	...
	if *sequential {
		for i, filename := range filenames {
			var err error
			if len(*liveFile) > 0 && *liveFile == filename {
				files[i], err = parseStdin(filename)
			} else {
...
func getPkgFiles(args []string) (files, xfiles []*ast.File, err error) {
	if len(args) == 0 {
		// stdin
		file, err := parseStdin("<standard input>")
		if err != nil {
			return nil, nil, err
		}
